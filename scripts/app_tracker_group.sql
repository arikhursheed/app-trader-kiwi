...
-- comtent_rating, ROI, USE FOR TOP 10

SELECT distinct content_rating_app as content_rating, 
				round(avg(profit),0) as avg_profit, 
				round(avg(ROUND(100*profit/(buy_cost + marketing_cost),2)), 2) as avg_ROI,
				count (*)
FROM Top_100 
GROUP BY content_rating
ORDER BY avg_profit desc, content_rating desc
